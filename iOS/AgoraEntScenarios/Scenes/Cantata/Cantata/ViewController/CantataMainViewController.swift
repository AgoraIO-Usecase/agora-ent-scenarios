...
//                }
        ...
        didSet {
            ...
            if oldValue != isNowMicMuted {
               // ktvApi.setMicStatus(isOnMicOpen: !isNowMicMuted)
                ktvApi.muteMic(muteStatus: isNowMicMuted)
    ...
    public override func viewDidLoad() {
        ...
        if isRoomOwner == true {
            ...
            ApiManager.shared.fetchStartCloud(mainChannel: roomNo, cloudRtcUid: 232425) {[weak self] flag in
    ...
    @objc func showDebug() {
        let presentView = LSTPopView.popDHCDebugView(withParentView: self.view, isDebugMode: self.isDumpAble, with: self)
        
    }
    
    @objc private func cloudFailed() {
        if self.isRoomOwner == false {return}
        let mes = "ktv_merge_failed_and create".toSceneLocalization() as String
        DispatchQueue.main.async {
            VLKTVAlert.shared().showKTVToast(withFrame: UIScreen.main.bounds, image: UIImage.sceneImage(name: "empty", bundleName: "DHCResource")!, message: mes, buttonTitle: "ktv_confirm".toSceneLocalization() as String) {[weak self] flag, text in
    ...
    @objc private func giveupRoom() {
        ...
        if self.isRoomOwner == false {return}
        let mes = "ktv_timeout_and_create".toSceneLocalization() as String
        DispatchQueue.main.async {
            VLKTVAlert.shared().showKTVToast(withFrame: UIScreen.main.bounds, image: UIImage.sceneImage(name: "empty", bundleName: "DHCResource")!, message: mes, buttonTitle: "ktv_confirm".toSceneLocalization() as String) {[weak self] flag, text in
                guard let self = self else {return}
                VLKTVAlert.shared().dismiss()
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        if let gestureRecognizers = self.navigationController?.interactivePopGestureRecognizer?.view?.gestureRecognizers {
            // 这里对添加到右滑视图上的所有手势禁用
...
//        jukeBoxView.backgroundColor = Color(hexString: "#152164")
    ...
    private func loadRtc() {
      ...
      //  isNowMicMuted = checkAudicMicIsMuted()
        ...
        self.RtcKit.setAudioEffectPreset(.roomAcousticsChorus)
    ...
    private func loadAndPlaySong() {
       ...
       // config.autoPlay = (role == .audience || role == .coSinger) ? false : true
        ...
        self.loadMusicCallBack = {[weak self] flag, songCode in
            ...
            self.ktvApi.switchSingerRole(newRole: role) { state, reason in
                if state != .success {
                    print("switch failed:\(role)----\(state.rawValue)")
                    return
                } else {
                    if role == .soloSinger || role == .leadSinger {
                        self.ktvApi.startSing(songCode: Int(model.songNo ?? "0") ?? 0, startPos: 0)
                    }
                }
            }
...
//        }
    ...
    private func joinChorus() {
        ...
        self.loadMusicCallBack = { [weak self] isSuccess, songCode in
            ...
            weakSelf.ktvApi.switchSingerRole(newRole: role) { state, reason in
                ...
                AppContext.dhcServiceImp().joinChorus(with: inputModel) { error in
                    // completion block
                }
...
//                }
    ...
    func enterSeat(withIndex index: Int, completion: @escaping (Error?) -> Void) {
        ...
        AppContext.dhcServiceImp().enterSeat(with: inputModel) { error in
            completion(error)
        }
    }
    ...
    private func leaveChorus() {
        //先下麦然后切换角色
        AppContext.dhcServiceImp().coSingerLeaveChorus { [weak self] error in
            guard let weakSelf = self else { return }
            
            weakSelf.stopPlaySong()
            weakSelf.isNowMicMuted = true
            weakSelf.lrcControlView.controlState = .joinChorus
            AppContext.dhcServiceImp().updateSeatAudioMuteStatus(with: true) { error in
                // completion block
            }
        }
    }
    ...
    private func markSong(with model: VLRoomSelSongModel) {
        ...
        AppContext.dhcServiceImp().markSongDidPlay(with: model, completion: { _ in
...
extension CantataMainViewController: IMusicLoadStateListener {
    public func onMusicLoadFail(songCode: Int, reason: KTVLoadSongFailReason) {
        ...
        if self.isNetWorkBad {
            VLToast.toast("ktv_bad_net".toSceneLocalization() as String, duration: 5.0)
        }
...
//            if self.singerRole == .soloSinger || self.singerRole == .leadSinger || self.isJoinChorus == true {
...
extension CantataMainViewController {
    func subscribeServiceEvent() {
        AppContext.dhcServiceImp().unsubscribeAll()

        AppContext.dhcServiceImp().subscribeUserListCountChanged {[weak self] count in
            // TODO
...
//                guard granted else { return }
                ...
                if status == .created {
                    ...
                    if VLUserCenter.user.id == seatModel.userNo && self.isLeavingChorus == false && self.singerRole == .coSinger {
                        DispatchQueue.main.async {
                            VLToast.toast("ktv_be_offline".toSceneLocalization() as String)
                            self.leaveSeat()
                            self.singerRole = .audience
                            self.lrcControlView.controlState = self.isRoomOwner ? .ownerChorus : .joinChorus
                            self.ktvApi.switchSingerRole(newRole: .audience) { _, _ in
...
//                    }
            ...
            if status == .updated {
               ...
               // self.choosedBgModel = selBgModel
                ...
                if roomInfo.creatorNo == VLUserCenter.user.id {
                    let mes = "ktv_room_exit".toSceneLocalization() as String
                    VLKTVAlert.shared().showKTVToast(withFrame: UIScreen.main.bounds, image: UIImage.sceneImage(name: "empty", bundleName: "DHCResource")!, message: mes, buttonTitle: "ktv_confirm".toSceneLocalization() as String) {[weak self] flag, text in
                        guard let self = self else {return}
                        VLKTVAlert.shared().dismiss()
                        self.leaveRoom()
        ...
        AppContext.dhcServiceImp().subscribeChooseSongChanged {[weak self] status, songInfo, songArray in
            guard let self = self else {return}
        ...
        AppContext.dhcServiceImp().subscribeNetworkStatusChanged {[weak self] status in
            guard let self = self else {return}
        ...
        AppContext.dhcServiceImp().subscribeRoomWillExpire {
            let isOwner = self.roomModel?.creatorNo == VLUserCenter.user.id
            let mes = isOwner ? "ktv_room_timeout".toSceneLocalization() as String : "ktv_room_offline".toSceneLocalization() as String
            VLKTVAlert.shared().showKTVToast(withFrame: UIScreen.main.bounds, image: UIImage.sceneImage(name: "empty", bundleName: "DHCResource")!, message: mes, buttonTitle: "ktv_confirm".toSceneLocalization() as String) {[weak self] flag, text in
                guard let self = self else {return}
                VLKTVAlert.shared().dismiss()
                self.leaveRoom()
    ...
    func leaveSeat(with seatModel: VLRoomSeatModel, completion:@escaping ((Error?) -> Void)) {
        ...
        AppContext.dhcServiceImp().leaveSeatWithoutRemoveSong(with: seatModel, completion: { err in
            completion(err)
        })
    ...
    private func enterSeatWithModel(_ index:Int, completion:@escaping ((Error?) -> Void)) {
        ...
        AppContext.dhcServiceImp().enterSeat(with: seatModel, completion: { err in
            completion(err)
        })
       // checkEnterSeatAudioAuthorized()
    }
...
//        isEnterSeatNotFirst = true
        AgoraEntAuthorizedManager.checkAudioAuthorized(parent: self) { flag in
            if flag {
                AppContext.dhcServiceImp().updateSeatAudioMuteStatus(with: false, completion: { err in
    ...
    func removeSelSong(songNo: Int, sync: Bool) -> Bool {
        ...
        if let removed = removed {
            ...
            if sync {
                ...
                AppContext.dhcServiceImp().removeSong(with: inputModel) { error in
                    if let error = error {
                        // cp todo
    ...
    func refreshChoosedSongList() {
        AppContext.dhcServiceImp().getChoosedSongsList {[weak self] (error, songArray) in
            guard let self = self else {return}
...
//        }
    ...
    private func changeToNextSong() {
        let title = "ktv_change_song".toSceneLocalization() as String
        let message = "ktv_change_next_song".toSceneLocalization() as String
        let array = ["ktv_cancel".toSceneLocalization() as String, "ktv_confirm".toSceneLocalization() as String]
    ...
    private func showSettingView() {
        let popView = LSTPopView.popDHCSettingView(withParentView: self.view, settingView: self.settingView, withDelegate: self)
...
extension CantataMainViewController: DHCGameDelegate {
    public func didGameEventChanged(with event: DHCGameEvent) {
        if event == .join {//加入合唱
            ...
            if isNetWorkBad {
                VLToast.toast("ktv_check_network".toSceneLocalization() as String)
                return
            }
            changeToNextSong()
...
//            realScore = self.cosingerDegree
        ...
        AppContext.dhcServiceImp().updateSeatScoreStatus(with: realScore, completion: { err in
            if err == nil {
            }
        })
    }
        ...
        if isLocal && singerRole == .leadSinger || singerRole == .soloSinger {
            if state == .playBackCompleted || state == .playBackAllLoopsCompleted {
                ...
                if let topsong = self.selSongArray?.first {
                    ...
                    AppContext.dhcServiceImp().updateSongEndStatus(with: true, inputModel: model, completion: { err in
...
extension CantataMainViewController: DHCVLKTVTopViewDelegate {
    ...
    public func onVLKTVTopView(_ view: DHCVLKTVTopView, closeBtnTapped sender: Any) {
        let title = VLUserCenter.user.ifMaster ? "ktv_disband_room".toSceneLocalization(with: "DHCResource") : "ktv_exit_room".toSceneLocalization(with: "DHCResource")
        let message = VLUserCenter.user.ifMaster ? "ktv_confirm_disband_room".toSceneLocalization(with: "DHCResource") : "ktv_confirm_exit_room".toSceneLocalization(with: "DHCResource")
        let array = ["ktv_cancel".toSceneLocalization(with: "DHCResource"), "ktv_confirm".toSceneLocalization(with: "DHCResource")]
        VLAlert.shared().show(withFrame: UIScreen.main.bounds, title: title as String, message: message as String, placeHolder: "", type: .ALERTYPENORMAL, buttonTitles: array) {[weak self] (flag, text) in
            if flag {
                self?.leaveRoom()
            }
            VLAlert.shared().dismiss()
...
extension CantataMainViewController: VLBottomViewDelegate {
    ...
    public func didBottomViewAudioStateChangeTo(enable: Bool) {
        ...
        AppContext.dhcServiceImp().updateSeatAudioMuteStatus(with: self.isNowMicMuted, completion: { err in
...
extension CantataMainViewController: VLDropOnLineViewDelegate {
    private func popDropLineView(with seatModel: VLRoomSeatModel) {
        LSTPopView.popDHCDropLineView(withParentView: self.view, with: seatModel, withDelegate: self)
    }
    ...
    public func onVLDrop(_ view: VLDropOnLineView, action seatModel: VLRoomSeatModel?) {
        ...
        leaveSeat(with: seatModel) { err in
            LSTPopView.getDHCPopView(withCustomView: view).dismiss()
        }
...
extension CantataMainViewController: DHCVLKTVSettingViewDelegate, VLEarSettingViewViewDelegate{
    ...
    private func showEarSettingView() {
        LSTPopView.popDHCEarSettingView(withParentView: self.view, isEarOn: self.isEarOn, vol: CGFloat(self.earValue), with: self)
    }
