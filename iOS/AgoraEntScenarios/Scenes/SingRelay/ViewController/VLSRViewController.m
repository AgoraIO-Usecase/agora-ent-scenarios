...
-(void)startSBGGrapWith:(int)index {
    VLSRRoomSelSongModel* model = [[self selSongsArray] firstObject];
    kWeakSelf(self);
    [[SBGNetworkManager shared] startSongGrab:[AppContext.shared appId] sceneId:@"sing_battle_game_info" roomId:_roomModel.roomNo headUrl:@"12345" userId:VLUserCenter.user.id userName:VLUserCenter.user.name songCode:model.songNo success:^(BOOL flag) {
        if(flag){
            //抢唱成功
            NSLog(@"抢唱成功");
...
-(NSDictionary *)convertScoreArrayToRank {
    ...
    //先进性数组合并
    NSArray *mergeModels = srMergeModelsWithSameUserIds(self.scoreArray);
...
NSArray<SRSubRankModel *> *srMergeModelsWithSameUserIds(NSArray<SRSubRankModel *> *models) {
...
-(void)setGameModel:(SingRelayModel *)gameModel {
    ...
    if(gameModel.status == SingRelayStatusWaiting){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.statusView hideNextMicOwner];
            NSArray *mergeModels = srMergeModelsWithSameUserIds(self.scoreArray);
            self.statusView.dataSource = srSortModels(mergeModels, NO);
...
NSArray<SRSubRankModel *> *srSortModels(NSArray<SRSubRankModel *> *models, BOOL ascending) {
    NSArray<SRSubRankModel *> *sortedByCountModels = srSortModelsByCountAndScore(models, ascending);
    NSArray<SRSubRankModel *> *resultModels = srAssignIndexesToModelsInArray(sortedByCountModels);
    return resultModels;
}

NSArray<SRSubRankModel *> *srSortModelsByCountAndScore(NSArray<SRSubRankModel *> *models, BOOL ascending) {
...
NSArray<SRSubRankModel *> *srAssignIndexesToModelsInArray(NSArray<SRSubRankModel *> *array) {
