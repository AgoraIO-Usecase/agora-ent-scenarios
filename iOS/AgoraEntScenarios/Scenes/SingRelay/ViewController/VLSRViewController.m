    ...
    [[AppContext srServiceImp] innerSingRelayInfo:^(NSError * error, SingRelayModel * model) {
        if(error == nil){
            if(model.status == SingRelayStatusStarted){
                ...
                [[VLKTVAlert shared]showKTVToastWithFrame:UIScreen.mainScreen.bounds image:[UIImage sceneImageWithName:@"empty" bundleName:@"SRResource"] message:mes buttonTitle:SRLocalizedString(@"sr_confirm") completion:^(bool flag, NSString * _Nullable text) {
...
- (void)popForceLeaveRoom {
    VL(weakSelf);
    [[VLKTVAlert shared]showKTVToastWithFrame:UIScreen.mainScreen.bounds image:[UIImage sceneImageWithName:@"empty" bundleName:@"SRResource"] message:SRLocalizedString(@"room_has_close") buttonTitle:SRLocalizedString(@"confirm") completion:^(bool flag, NSString * _Nullable text) {
