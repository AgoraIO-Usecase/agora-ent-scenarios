...
/// 加载歌曲状态
@objc public enum SRLoadSongState: Int {
    case idle = -1      //空闲
    case ok = 0         //成功
    case failed         //失败
    case inProgress    //加载中
...
private func agoraPrint(_ message: String) {
   // SRLog.info(text: message, tag: "SRApi")
}

class SRApiImpl: NSObject{
    
    private var apiConfig: SRApiConfig?

    private var songConfig: SRSongConfiguration?
    ...
    private var loadSongMap = Dictionary<String, SRLoadSongState>()
    private var lyricUrlMap = Dictionary<String, String>()
    private var loadDict = Dictionary<String, SRLoadSongState>()
    ...
    private var loadSongState: SRLoadSongState = .idle
    ...
    private var songMode: SRSongMode = .songCode
    private var useCustomAudioSource:Bool = false
    private var songUrl: String = ""
    private var songCode: Int = 0
    private var songIdentifier: String = ""

    private var singerRole: SRSingRole = .audience {
        didSet {
            agoraPrint("singerRole changed: \(oldValue.rawValue)->\(singerRole.rawValue)")
        }
    }
    private var lrcControl: SRLrcViewDelegate?
    
    private var timer: Timer?
    private var isPause: Bool = false
    
    public var remoteVolume: Int = 40
    private var joinChorusNewRole: SRSingRole = .audience
    private var oldPitch: Double = 0
    deinit {
        mcc?.register(nil)
        agoraPrint("deinit SRApiImpl")
    }

    @objc required init(config: SRApiConfig) {
        super.init()
        agoraPrint("init SRApiImpl")
...
//MARK: SRApiDelegate
extension SRApiImpl: SRApiDelegate {
    ...
    func addEventHandler(SRApiEventHandler: SRApiEventHandlerDelegate) {
        if eventHandlers.contains(SRApiEventHandler) {
            return
        }
        eventHandlers.add(SRApiEventHandler)
    }

    func removeEventHandler(SRApiEventHandler: SRApiEventHandlerDelegate) {
        eventHandlers.remove(SRApiEventHandler)
    }
    ...
    func switchSingerRole(newRole: SRSingRole, onSwitchRoleState: @escaping (SRSwitchRoleState, SRSwitchRoleFailReason) -> Void) {
     ...
     * 选择音轨，原唱、伴唱
     */
//    @objc public func selectPlayerTrackMode(mode: SRPlayerTrackMode) {
...
extension SRApiImpl {
    private func switchSingerRole(oldRole: SRSingRole, newRole: SRSingRole, token: String, stateCallBack:@escaping ISRSwitchRoleStateListener) {
     ...
     * 加入合唱
     */
    private func joinChorus(role: SRSingRole, token: String, joinExChannelCallBack: @escaping JoinExChannelCallBack) {
        self.onJoinExChannelCallBack = joinExChannelCallBack
        if role == .leadSinger {
            agoraPrint("joinChorus: SRSingRoleMainSinger")
    ...
    private func joinChorus2ndChannel(newRole: SRSingRole, token: String) {
        let role = newRole
    ...
    private func leaveChorus2ndChannel(_ role: SRSingRole) {
    ...
    private func leaveChorus(role: SRSingRole) {
        ...
        if role == .leadSinger {
         ...
         //   mediaOption.autoSubscribeVideo = false
            ...
            agoraPrint("joinChorus: SRSingRoleAudience does not need to leaveChorus!")
        }
    }
}
...
extension SRApiImpl {
    ...
    private func getEventHander(callBack:((SRApiEventHandlerDelegate)-> Void)) {
        for obj in eventHandlers.allObjects {
            if obj is SRApiEventHandlerDelegate {
                callBack(obj as! SRApiEventHandlerDelegate)
            }
        }
    }
    
    private func _loadMusic(config: SRSongConfiguration, mode: SRLoadMusicMode, onMusicLoadStateListener: ISRMusicLoadStateListener){
...
// rtc的代理回调
extension SRApiImpl: AgoraRtcEngineDelegate {
...
extension SRApiImpl {
    ...
    @objc public func didSRAPIReceiveStreamMessageFrom(uid: NSInteger, streamId: NSInteger, data: Data) {
    ...
    private func handleSetLrcTimeCommand(dict: [String: Any], role: SRSingRole) {
...
//        self.remotePlayerPosition = TimeInterval(realPosition)
    ...
    @objc public func didSRAPIReceiveAudioVolumeIndication(with speakers: [AgoraRtcAudioVolumeInfo], totalVolume: NSInteger) {
    ...
    @objc public func didSRAPILocalAudioStats(stats: AgoraRtcLocalAudioStats) {
...
//主要是MPK的回调
extension SRApiImpl: AgoraRtcMediaPlayerDelegate {
...
//主要是MCC的回调
extension SRApiImpl: AgoraMusicContentCenterEventDelegate {
    ...
    func onPreLoadEvent(_ requestId: String, songCode: Int, percent: Int, lyricUrl: String?, status: AgoraMusicContentCenterPreloadStatus, errorCode: AgoraMusicContentCenterStatusCode) {
        if let listener = self.loadMusicListeners.object(forKey: "\(songCode)" as NSString) as? ISRMusicLoadStateListener {
