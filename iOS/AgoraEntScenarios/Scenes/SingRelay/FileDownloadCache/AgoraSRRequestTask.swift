...
class AgoraSRRequestTask: NSObject {
    public weak var delegate: AgoraSRLrcDownloadDelegate?
    // 是否缓存
    var cache: Bool = true {
        didSet {
            task?.cancel()
...
extension AgoraSRRequestTask: URLSessionDownloadDelegate {
    func urlSession(_: URLSession, downloadTask _: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        // 可以缓存则保存文件
        guard cache == true, let url = requestURL else { return }
        AgoraSRCacheFileHandle.moveFile(with: location.path,
