...
@objc(AgoraSRLrcDownloadDelegate)
public
protocol AgoraSRLrcDownloadDelegate {
    /// 开始下载
    @objc
...
@objc class AgoraSRDownLoadManager: NSObject {
    ...
    @objc public weak var delegate: AgoraSRLrcDownloadDelegate?
                         ...
                         failure: @escaping UnZipErrorClosure)
    {
        ...
        let xmlPath = AgoraSRCacheFileHandle.cacheFileExists(with: fileName + ".xml")
        let lrcPath = AgoraSRCacheFileHandle.cacheFileExists(with: urlString)
    ...
    func downloadMP3(urlString: String, success: @escaping Sunccess) {
        self.urlString = urlString
        let cachePath = AgoraSRCacheFileHandle.cacheFileExists(with: urlString)
        if cachePath == nil {
            guard let url = URL(string: urlString) else { return }
            request.delegate = self
            request.download(requestURL: url)
...
extension AgoraSRDownLoadManager: AgoraSRLrcDownloadDelegate {
