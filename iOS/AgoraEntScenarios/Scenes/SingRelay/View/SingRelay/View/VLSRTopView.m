...
- (void)setupView {
    ...
    logoImgView.image = [UIImage sceneImageWithName:@"" bundleName:@"SRResource"];
    ...
    [closeBtn setImage:[UIImage sceneImageWithName:@"close_room" bundleName:@"SRResource"] forState:UIControlStateNormal];
    [closeBtn addTarget:self action:@selector(closeBtnEvent:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:closeBtn];
    
    UIButton *moreButton = [[UIButton alloc] init];
    [moreButton setImage:[UIImage sceneImageWithName:@"icon_live_more" bundleName:@"SRResource"] forState:(UIControlStateNormal)];
    ...
    [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_wellIcon" bundleName:@"SRResource"] forState:UIControlStateNormal];
...
- (void)setNetworkQuality:(int)quality
{
    if(quality == 0) {
        ...
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_okIcon" bundleName:@"SRResource"] forState:UIControlStateNormal];
        [self.networkStatusBtn setTitle:SRLocalizedString(@"sr_net_status_m") forState:UIControlStateNormal];
    }
    else if(quality == 2) {
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_badIcon" bundleName:@"SRResource"] forState:UIControlStateNormal];
