...
@interface VLSRSearchSongResultView()<
UITableViewDataSource,
UITableViewDelegate
>

@property(nonatomic, weak) id <VLSRSearchSongResultViewDelegate>delegate;
...
@implementation VLSRSearchSongResultView

- (void)dealloc {
}

- (instancetype)initWithFrame:(CGRect)frame
                 withDelegate:(id<VLSRSearchSongResultViewDelegate>)delegate
...
- (void)loadSearchDataWithKeyWord:(NSString *)keyWord ifRefresh:(BOOL)ifRefresh {
    ...
    [[AppContext shared].srAPI searchMusicWithKeyword:keyWord ? keyWord : @""
                                                  page:self.page
                                              pageSize:5
                                            jsonOption:extra
                                           completion:^(NSString * requestId, AgoraMusicContentCenterStateReason status, AgoraMusicCollection * result) {
