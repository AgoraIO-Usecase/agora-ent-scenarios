...
class VLSRStatusView: UIView {
    ...
    private var basicScoreModels  = {
        ...
        var model2 = SRSubRankModel()
        model2.index = 2
        
        var model3 = SRSubRankModel()
        model3.index = 3
   ...
   @objc var state: SRState = .ownerOrderMusic {
        didSet {
    ...
    private lazy var orderBtn: UIButton = { //点歌按钮
        ...
        btn.addTarget(self, action: #selector(startSr), for: .touchUpInside)
        return btn
    }()
    ...
    @objc public lazy var srBtn: UIButton = { //抢唱按钮
        ...
        btn.addTarget(self, action: #selector(sr), for: .touchUpInside)
        return btn
    }()
