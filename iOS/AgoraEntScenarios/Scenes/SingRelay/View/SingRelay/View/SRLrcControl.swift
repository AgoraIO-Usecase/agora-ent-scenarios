...
@objc public protocol SRLrcControlDelegate: NSObjectProtocol {
...
@objc class SRLrcControl: NSObject {

    @objc public weak var lrcView: KaraokeView?
    @objc public weak var delegate: SRLrcControlDelegate?
    @objc public var skipCallBack: ((Int, Bool) -> Void)?

    private var skipBtn: SRSkipView!
  ...
  //  private var downloadManager: AgoraDownLoadManager = AgoraDownLoadManager()
    ...
    private func setupSkipBtn() {
        let frame = CGRect(x: (lrcView?.bounds.size.width ?? 0) / 2.0 - 60,
                           ...
                           width: 120,
                           height: 34)
        skipBtn = SRSkipView(frame: frame) { [weak self] type in
            guard let self = self,
...
extension SRLrcControl: KaraokeDelegate {
...
//extension SRLrcControl: AgoraLrcDownloadDelegate {
...
extension SRLrcControl: LyricsFileDownloaderDelegate {
