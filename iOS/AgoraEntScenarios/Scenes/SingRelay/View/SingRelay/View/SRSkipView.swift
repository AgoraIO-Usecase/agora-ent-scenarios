...
typealias OnSRSkipCallback = (SRSkipActionType) -> Void

class SRSkipView: UIView {
    
    private var bgView: UIView!
    private var skipBtn: UIButton!
    private var canCelBtn: UIButton!
    private var completion: OnSRSkipCallback?
    
    init(frame: CGRect, completion: OnSRSkipCallback?) {
    ...
    @objc private func skip(_ btn: UIButton) {
        let actionType: SRSkipActionType = (btn.tag == 200) ? .down : .cancel
