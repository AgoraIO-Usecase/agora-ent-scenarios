...
@interface VLSRMVView () <VLSRMVIdleViewDelegate, KaraokeDelegate>

@property(nonatomic, weak) id <VLSRMVViewDelegate>delegate;
...
@property (nonatomic, strong) VLSRMVIdleView *idleView;//没有人演唱视图
...
- (void)setIsPlayAccompany:(BOOL)isPlayAccompany {
    ...
    VLSRMVViewActionType targetOrigin = isPlayAccompany ? VLSRMVViewActionTypeSingAcc : VLSRMVViewActionTypeSingOrigin;
    if ([self.delegate respondsToSelector:@selector(onSRMVView:btnTappedWithActionType:)]) {
        [self.delegate onSRMVView:self btnTappedWithActionType:targetOrigin];
    }
}

- (void)setLoadingType:(VLSRMVLoadingState)loadingType {
    _loadingType = loadingType;
    
    if (loadingType == VLSRMVViewStateLoading) {
    ...
    } else if (loadingType == VLSRMVViewStateLoadFail) {
    ...
    } else if (loadingType == VLSRMVViewStateIdle){
        [self.loadingView stopAnimating];
        [self.loadingView setHidden:YES];
        [self.contentView setHidden:NO];
        [self.retryButton setHidden:YES];
...
- (void)layoutSubviews {
    ...
    CGFloat tipsTop = self.loadingType == VLSRMVViewStateLoading ? self.loadingView.bottom + 5 : self.incentiveView.bottom + 10;
...
- (void)setupView {
   ...
   // _karaokeView.scoringView.showDebugView = true;
    ...
    self.idleView = [[VLSRMVIdleView alloc]initWithFrame:CGRectMake(0, 0, self.width, self.height) withDelegate:self];
...
- (void)_refreshOriginButton {
    if (self.originBtn.selected) {
        [self.originBtn setTitle:SRLocalizedString(@"ktv_room_original") forState:UIControlStateNormal];
        [self.originBtn setTitle:SRLocalizedString(@"ktv_room_original") forState:UIControlStateSelected];
    } else {
        [self.originBtn setTitle:SRLocalizedString(@"ktv_room_original") forState:UIControlStateNormal];
        [self.originBtn setTitle:SRLocalizedString(@"ktv_room_original") forState:UIControlStateSelected];
    }
...
- (void)updateMVPlayerState:(VLSRMVViewActionType)state {
    if (state == VLSRMVViewActionTypeMVPlay) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.pauseBtn.selected = YES;
        });
    } else if (state == VLSRMVViewActionTypeMVPause) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)onRetryAction:(UIButton*)button {
    if ([self.delegate respondsToSelector:@selector(onSRMVView:btnTappedWithActionType:)]) {
        [self.delegate onSRMVView:self btnTappedWithActionType:VLSRMVViewActionTypeRetryLrc];
    }
}
...
- (void)playClick:(UIButton *)button {
    ...
    if ([self.delegate respondsToSelector:@selector(onSRMVView:btnTappedWithActionType:)]) {
        if (button.selected) {
            [self.delegate onSRMVView:self btnTappedWithActionType:VLSRMVViewActionTypeMVPlay];
        } else {
            [self.delegate onSRMVView:self btnTappedWithActionType:VLSRMVViewActionTypeMVPause];
        }
    }
}
...
- (void)buttonClick:(UIButton *)sender {
    ...
    if (sender == self.settingBtn) {
        if ([self.delegate respondsToSelector:@selector(onSRMVView:btnTappedWithActionType:)]) {
            [self.delegate onSRMVView:self btnTappedWithActionType:VLSRMVViewActionTypeSetParam];
        }
    } else if (sender == self.nextButton) {
        if ([self.delegate respondsToSelector:@selector(onSRMVView:btnTappedWithActionType:)]) {
            [self.delegate onSRMVView:self btnTappedWithActionType:VLSRMVViewActionTypeMVNext];
        }
    }
}
...
- (void)configPlayerControls:(VLSRRoomSelSongModel *)song role:(SRSingRole)role {
    // 是主唱/伴唱
    switch (role) {
        ...
        case SRSingRoleLeadSinger: {
            ...
            self.pauseBtn.frame = CGRectMake(20, 0, 34, 54);
            self.nextButton.frame = CGRectMake(_pauseBtn.right+10, _pauseBtn.top, 34, 54);
          //  self.joinCoSingerState = SRJoinCoSingerStateIdle;
        } break;
...
//        case SRSingRoleFollowSinger:
         ...
         //   self.joinCoSingerState = SRJoinCoSingerStateWaitingForLeave;
        } break;
        ...
        default: {
           ...
           // self.joinCoSingerState = SRJoinCoSingerStateWaitingForJoin;
        } break;
    }
}
...
- (BOOL)isPlaying:(VLSRRoomSelSongModel *)song {
    if (song.status == VLSRSongPlayStatusPlaying) {
        return YES;
    }
    return NO;
}
...
//    SRLogInfo(@"VLSRMVView updateUIWithSong: songName: %@, name: %@, role: %ld", song.songName, song.name, role);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    _originBtn.selected = type == VLSRMVViewActionTypeSingOrigin ? YES : NO;
    [self _refreshOriginButton];
}

- (void)originClick:(UIButton *)button {
    BOOL targetState = !button.selected;
    VLSRMVViewActionType targetOrigin = targetState ? VLSRMVViewActionTypeSingOrigin : VLSRMVViewActionTypeSingAcc;
...
- (void)reset {
    //SRLogInfo(@"VLSRMVView reset [%@]", self.musicTitleLabel.text);
    [self setSongScore:0];
...
-(void)agoraKaraokeScoreWithScore:(double)score cumulativeScore:(double)cumulativeScore totalScore:(double)totalScore {
    ...
    if ([self.delegate respondsToSelector:@selector(onSRMVView:scoreDidUpdate:)]) {
        [self.delegate onSRMVView:self scoreDidUpdate:realScore];
    }
}
...
- (UIButton *)originBtn {
    if (!_originBtn) {
        ...
        [_originBtn setTitle:SRLocalizedString(@"ktv_room_original") forState:UIControlStateNormal];
...
- (VLHotSpotBtn *)settingBtn {
    if (!_settingBtn) {
        ...
        [self.settingBtn setTitle:SRLocalizedString(@"ktv_room_player_tweak") forState:UIControlStateNormal];
...
- (UILabel *)scoreLabel {
    if (!_scoreLabel) {
        ...
        _scoreLabel.text = SRLocalizedString(@"0");
    }
    return _scoreLabel;
}
...
- (UILabel *)scoreUnitLabel {
    if (!_scoreUnitLabel) {
        ...
        _scoreUnitLabel.text = SRLocalizedString(@"ktv_score_formatter");
    }
