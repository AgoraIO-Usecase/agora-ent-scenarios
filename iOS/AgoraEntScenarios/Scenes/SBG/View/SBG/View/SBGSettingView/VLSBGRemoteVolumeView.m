...
@interface VLSBGRemoteVolumeView() <UITextFieldDelegate>
...
@implementation VLSBGRemoteVolumeView
...
- (UIButton *)addButton {
    if (!_addButton) {
        ...
        [_addButton setImage:[UIImage sceneImageWithName:@"icon_ktv_add" bundleName:@"sbgResource"] forState:UIControlStateNormal];
...
- (UIButton *)reduceButton {
    if (!_reduceButton) {
        ...
        [_reduceButton setImage:[UIImage sceneImageWithName:@"icon_ktv_reduce" bundleName:@"sbgResource"] forState:UIControlStateNormal];
...
- (id)initWithMin:(int)min withMax:(int)max withCurrent:(int)current {
    if (self = [super init]) {
        ...
        [self initSubViews];
        self.inputTipsLabel.text = [NSString stringWithFormat:SBGLocalizedString(@"RemoteVolumeTipsFormat"), min, max];
