...
- (void)setupView {
    ...
    logoImgView.image = [UIImage sceneImageWithName:@"" bundleName:@"sbgResource"];
    ...
    [closeBtn setImage:[UIImage sceneImageWithName:@"close_room" bundleName:@"sbgResource"] forState:UIControlStateNormal];
    [closeBtn addTarget:self action:@selector(closeBtnEvent:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:closeBtn];
    
    UIButton *moreButton = [[UIButton alloc] init];
    [moreButton setImage:[UIImage sceneImageWithName:@"icon_live_more" bundleName:@"sbgResource"] forState:(UIControlStateNormal)];
    ...
    [self.networkStatusBtn setTitle:SBGLocalizedString(@"sbg_net_status_good") forState:UIControlStateNormal];
    [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_wellIcon" bundleName:@"sbgResource"] forState:UIControlStateNormal];
...
- (void)setNetworkQuality:(int)quality
{
    if(quality == 0) {
        ...
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_okIcon" bundleName:@"sbgResource"] forState:UIControlStateNormal];
        [self.networkStatusBtn setTitle:SBGLocalizedString(@"sbg_net_status_m") forState:UIControlStateNormal];
    }
    else if(quality == 2) {
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_badIcon" bundleName:@"sbgResource"] forState:UIControlStateNormal];
        [self.networkStatusBtn setTitle:SBGLocalizedString(@"sbg_net_status_low") forState:UIControlStateNormal];
    }
    else {
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_wellIcon" bundleName:@"sbgResource"] forState:UIControlStateNormal];
        [self.networkStatusBtn setTitle:SBGLocalizedString(@"sbg_net_status_good") forState:UIControlStateNormal];
