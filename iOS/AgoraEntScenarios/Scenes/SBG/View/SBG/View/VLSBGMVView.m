...
@interface VLSBGMVView () <VLSBGMVIdleViewDelegate, KaraokeDelegate>

@property(nonatomic, weak) id <VLSBGMVViewDelegate>delegate;
...
@property (nonatomic, strong) VLSBGMVIdleView *idleView;//没有人演唱视图
...
- (void)setIsPlayAccompany:(BOOL)isPlayAccompany {
    ...
    VLSBGMVViewActionType targetOrigin = isPlayAccompany ? VLSBGMVViewActionTypeSingAcc : VLSBGMVViewActionTypeSingOrigin;
    if ([self.delegate respondsToSelector:@selector(onRSMVView:btnTappedWithActionType:)]) {
        [self.delegate onRSMVView:self btnTappedWithActionType:targetOrigin];
    }
}

- (void)setLoadingType:(VLSBGMVLoadingState)loadingType {
    _loadingType = loadingType;
    
    if (loadingType == VLSBGMVViewStateLoading) {
    ...
    } else if (loadingType == VLSBGMVViewStateLoadFail) {
    ...
    } else if (loadingType == VLSBGMVViewStateIdle){
        [self.loadingView stopAnimating];
        [self.loadingView setHidden:YES];
        [self.contentView setHidden:NO];
        [self.retryButton setHidden:YES];
...
- (void)layoutSubviews {
    ...
    CGFloat tipsTop = self.loadingType == VLSBGMVViewStateLoading ? self.loadingView.bottom + 5 : self.incentiveView.bottom + 10;
...
- (void)setupView {
   ...
   // _karaokeView.scoringView.showDebugView = true;
    ...
    self.idleView = [[VLSBGMVIdleView alloc]initWithFrame:CGRectMake(0, 0, self.width, self.height) withDelegate:self];
...
- (void)_refreshOriginButton {
    if (self.originBtn.selected) {
        [self.originBtn setTitle:SBGLocalizedString(@"原唱") forState:UIControlStateNormal];
        [self.originBtn setTitle:SBGLocalizedString(@"原唱") forState:UIControlStateSelected];
    } else {
        [self.originBtn setTitle:SBGLocalizedString(@"原唱") forState:UIControlStateNormal];
        [self.originBtn setTitle:SBGLocalizedString(@"原唱") forState:UIControlStateSelected];
    }
...
- (void)updateMVPlayerState:(VLSBGMVViewActionType)state {
    if (state == VLSBGMVViewActionTypeMVPlay) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.pauseBtn.selected = YES;
        });
    } else if (state == VLSBGMVViewActionTypeMVPause) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)onRetryAction:(UIButton*)button {
    if ([self.delegate respondsToSelector:@selector(onRSMVView:btnTappedWithActionType:)]) {
        [self.delegate onRSMVView:self btnTappedWithActionType:VLSBGMVViewActionTypeRetryLrc];
    }
}
...
- (void)playClick:(UIButton *)button {
    ...
    if ([self.delegate respondsToSelector:@selector(onRSMVView:btnTappedWithActionType:)]) {
        if (button.selected) {
            [self.delegate onRSMVView:self btnTappedWithActionType:VLSBGMVViewActionTypeMVPlay];
        } else {
            [self.delegate onRSMVView:self btnTappedWithActionType:VLSBGMVViewActionTypeMVPause];
        }
    }
}
...
- (void)buttonClick:(UIButton *)sender {
    ...
    if (sender == self.settingBtn) {
        if ([self.delegate respondsToSelector:@selector(onRSMVView:btnTappedWithActionType:)]) {
            [self.delegate onRSMVView:self btnTappedWithActionType:VLSBGMVViewActionTypeSetParam];
        }
    } else if (sender == self.nextButton) {
        if ([self.delegate respondsToSelector:@selector(onRSMVView:btnTappedWithActionType:)]) {
            [self.delegate onRSMVView:self btnTappedWithActionType:VLSBGMVViewActionTypeMVNext];
        }
    }
}
...
- (void)configPlayerControls:(VLSBGRoomSelSongModel *)song role:(SBGSingRole)role {
    // 是主唱/伴唱
    switch (role) {
        ...
        case SBGSingRoleLeadSinger: {
            ...
            self.pauseBtn.frame = CGRectMake(20, 0, 34, 54);
            self.nextButton.frame = CGRectMake(_pauseBtn.right+10, _pauseBtn.top, 34, 54);
          //  self.joinCoSingerState = RSJoinCoSingerStateIdle;
        } break;
...
//        case SBGSingRoleFollowSinger:
         ...
         //   self.joinCoSingerState = RSJoinCoSingerStateWaitingForLeave;
        } break;
        ...
        default: {
           ...
           // self.joinCoSingerState = RSJoinCoSingerStateWaitingForJoin;
        } break;
    }
}
...
- (BOOL)isPlaying:(VLSBGRoomSelSongModel *)song {
    if (song.status == VLSBGSongPlayStatusPlaying) {
        return YES;
    }
    return NO;
}
...
- (void)updateUIWithSong:(VLSBGRoomSelSongModel * __nullable)song role:(SBGSingRole)role {
    SBGLogInfo(@"VLSBGMVView updateUIWithSong: songName: %@, name: %@, role: %ld", song.songName, song.name, role);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    _originBtn.selected = type == VLSBGMVViewActionTypeSingOrigin ? YES : NO;
    [self _refreshOriginButton];
}

- (void)originClick:(UIButton *)button {
    BOOL targetState = !button.selected;
    VLSBGMVViewActionType targetOrigin = targetState ? VLSBGMVViewActionTypeSingOrigin : VLSBGMVViewActionTypeSingAcc;
...
- (void)reset {
    SBGLogInfo(@"VLSBGMVView reset [%@]", self.musicTitleLabel.text);
    [self setSongScore:0];
...
-(void)agoraKaraokeScoreWithScore:(double)score cumulativeScore:(double)cumulativeScore totalScore:(double)totalScore {
    ...
    if ([self.delegate respondsToSelector:@selector(onRSMVView:scoreDidUpdate:)]) {
        [self.delegate onRSMVView:self scoreDidUpdate:realScore];
    }
}
...
- (UIButton *)originBtn {
    if (!_originBtn) {
        ...
        [_originBtn setTitle:SBGLocalizedString(@"原唱") forState:UIControlStateNormal];
...
- (UILabel *)scoreLabel {
    if (!_scoreLabel) {
        ...
        _scoreLabel.text = SBGLocalizedString(@"0");
    }
    return _scoreLabel;
}
...
- (UILabel *)scoreUnitLabel {
    if (!_scoreUnitLabel) {
        ...
        _scoreUnitLabel.text = SBGLocalizedString(@"分");
    }
