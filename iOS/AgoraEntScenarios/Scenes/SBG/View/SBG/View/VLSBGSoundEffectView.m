//
//  VLSBGsoundEffectView.m
//  VoiceOnLine
//

#import "VLSBGsoundEffectView.h"
...
@interface VLSBGSoundEffectView ()

@property(nonatomic, weak) id <VLSBGsoundEffectViewDelegate>delegate;
...
@property (nonatomic, assign) VLSBGSoundEffectType typeValue;

@end


@implementation VLSBGSoundEffectView
...
- (void)setupView {
    ...
    titleLabel.text = SBGLocalizedString(@"音效");
    ...
    electronicLabel.text = SBGLocalizedString(@"启用电音");
...
//                                                   ballSize:22];
    ...
    typeLabel.text = SBGLocalizedString(@"选择调式");
...
//    [self.rotateImgView addGestureRecognizer:tapGes];
    ...
    self.heFengLabel.text = SBGLocalizedString(@"和风");
    ...
    self.xiaoDiaoLabel.text = SBGLocalizedString(@"小调");
    ...
    self.daDiaoLabel.text = SBGLocalizedString(@"大调");
...
- (void)closeEffect {
    if (!self.openSwitch.isOn) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(soundEffectItemClickAction:)]) {
            [self.delegate soundEffectItemClickAction:VLSBGSoundEffectTypeNone];
        }
    }
}
...
{
    if(self.typeValue == -1) {
        self.typeValue = VLSBGSoundEffectTypeHeFeng;
    }
...
- (void)itemBtnClick:(VLHotSpotBtn *)sender {
    ...
    if (sender.tag == 0) {
        ...
        [self rotateAnimationFrom:self.beforeValue toValue:self.rotateValue];
        self.typeValue = VLSBGSoundEffectTypeXiaoDiao;
        ...
        [self rotateAnimationFrom:self.beforeValue toValue:self.rotateValue];
        self.typeValue = VLSBGSoundEffectTypeDaDiao;
