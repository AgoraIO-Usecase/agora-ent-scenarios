...
struct ShowStatisticsInfo {
    ...
    mutating func cleanRemoteDescription() -> (String, String){
        ...
        let lastmile = "show_statistic_delay".pure1v1Localization() + ": \(0) ms"
    ...
    private func localDescription(info: LocalInfo, audioOnly: Bool) -> (String, String) {
        let sendTitle = "show_statistic_send_title".pure1v1Localization() + "\n"
        let videoSize = "show_statistic_encode_resolution".pure1v1Localization() + ": \(info.videoStats.encodedFrameWidth) x \(info.videoStats.encodedFrameHeight)"
        let videoSend = "show_statistic_up_bitrate".pure1v1Localization() + ": \(info.videoStats.sentBitrate) kbps"
        let uplink = "show_statistic_up_net_speech".pure1v1Localization() + ": \(uplink) KB/s"
        
        let fps = "show_advance_setting_FPS_title".pure1v1Localization() + ": \(fps) fps"
        let vSendLoss = "show_statistic_up_loss_package".pure1v1Localization() + ": \(info.videoStats.txPacketLossRate) %"
    ...
    private func remoteDescription(info: RemoteInfo, audioOnly: Bool) -> (String, String) {
        let sendTitle = "show_statistic_receive_title".pure1v1Localization() + "\n"
        let videoSize = "show_statistic_receive_resolution".pure1v1Localization() + ": \(info.videoStats.width) x \(info.videoStats.height)"
        let videoSend = "show_statistic_bitrate".pure1v1Localization() + ": \(info.videoStats.receivedBitrate) kbps"
        let downlink = "show_statistic_down_net_speech".pure1v1Localization() + ": \(downlink) KB/s"

        let fps = "show_statistic_receive_fps".pure1v1Localization() + ": \(fps) fps"
        let vSendLoss = "show_statistic_down_loss_package".pure1v1Localization() + ": \(info.videoStats.packetLossRate) %"
        let lastmile = "show_statistic_delay".pure1v1Localization() + ": \(info.videoStats.delay) ms"
