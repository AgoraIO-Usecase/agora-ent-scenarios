...
//                "testPort": 4447
            ...
            let task = session.dataTask(with: request) { (data, response, error) in
                if let error = error {
                    Pure1v1Logger.info("RttApiManager getToken error: \(error.localizedDescription)")
                    DispatchQueue.main.async {
                        completion(nil)
                    }
                    ...
                    do {
                        guard let responseDict = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any], let tokenName = responseDict["tokenName"] as? String else {
                            Pure1v1Logger.info("RttApiManager getToken error: \(data)")
                            return
                        }
                        DispatchQueue.main.async {
                            completion(tokenName)
                        }
                        Pure1v1Logger.info("RttApiManager getToken success")
                    } catch {
                        Pure1v1Logger.info("RttApiManager getToken error: \(error.localizedDescription)")
                        DispatchQueue.main.async {
                            completion(nil)
    ...
    func fetchStartRtt(
        ...
        self.fetchCloudToken { token in
            if token == nil {
                Pure1v1Logger.info("RttApiManager fetchStartRtt failed token is null")
                completion(false)
                return
            } else {
            ...
            do {
                ...
                Pure1v1Logger.info("RttApiManager fetchStartRtt url: \(request.httpBody)")
                
             //   let semaphore = DispatchSemaphore(value: 0)
                
                let task = self.session.dataTask(with: request) { (data, response, error) in
                    if let error = error {
                        Pure1v1Logger.info("RttApiManager fetchStartRtt failed: \(error.localizedDescription)")
                        DispatchQueue.main.async {
                            completion(false)
                        }
                    } else if let data = data {
                        do {
                            ...
                            self.taskId = taskId
                            Pure1v1Logger.info("RttApiManager fetchStartRtt success taskId: \(taskId)")
                            DispatchQueue.main.async {
                                completion(true)
                            }
                        } catch {
    ...
    func fetchStopRtt(completion: @escaping ((Bool)->Void)) {
        if taskId.isEmpty || tokenName.isEmpty {
            Pure1v1Logger.info("RttApiManager fetchStopRtt failed taskId || tokenName is null")
            return
        }
        ...
        do {
            ...
            let task = session.dataTask(with: request) { (data, response, error) in
                // Handle response
                Pure1v1Logger.info("RttApiManager fetchStopRtt: \(error)")
                if let error = error {
                    DispatchQueue.main.async {
                        completion(false)
                    }
            ...
            Pure1v1Logger.info("RttApiManager fetchStopRtt failed: \(error.localizedDescription)")
            DispatchQueue.main.async {
                completion(false)
            }
        }
