...
//        self.requestRankList()
    ...
    func notifyHorizontalTextCarousel(gift: VoiceRoomGiftEntity) {
        let string = NSAttributedString {
            ...
            AttributedText("gifts".voice_localized()).font(.systemFont(ofSize: 12, weight: .medium)).foregroundColor(.white)
            ...
            AttributedText("a rocket".voice_localized()).font(.systemFont(ofSize: 12, weight: .medium)).foregroundColor(.white)
        }
    ...
    func onUserJoinedRoom(roomId: String, user: VRUser) {
        ...
        self.convertShowText(userName: user.name ?? "", content: "Joined".voice_localized(), joined: true)
    }
    
    func onAnnouncementChanged(roomId: String, content: String) {
        self.view.makeToast("Voice room announcement changed!".voice_localized(), point: toastPoint, title: nil, image: nil, completion: nil)
...
//                }
                ...
                if let host: VRUser = roomInfo?.room?.owner {
                    if host.chat_uid == fromId, first.member == nil,changeMic.member?.uid ?? "" == VoiceRoomUserInfo.shared.user?.uid ?? "" {
                        ...
                        view.makeToast("You were removed from stage".voice_localized())
    ...
    func sendTextMessage(text: String) {
        ...
        VoiceRoomIMManager.shared?.sendMessage(roomId: roomId, text: text, ext: ["userName": userName]) { message, error in
            if error != nil,error?.code == .moderationFailed {
                self.view.makeToast("Content prohibited".voice_localized(), point: self.toastPoint, title: nil, image: nil, completion: nil)
            }
        }
    }
