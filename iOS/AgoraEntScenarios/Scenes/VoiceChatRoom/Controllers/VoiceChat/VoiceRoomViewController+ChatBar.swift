...
//
    ...
    func requestSpeak(index: Int?) {
        ChatRoomServiceImp.getSharedInstance().submitApply(index: index) { error, flag in
            if error == nil {
                if flag {
...
//        }
    ...
    func cancelRequestSpeak(index: Int?) {
        ...
        ChatRoomServiceImp.getSharedInstance().cancelApply(chat_uid: user.chat_uid ?? "") { error, flag in
            if error == nil {
                if flag {
