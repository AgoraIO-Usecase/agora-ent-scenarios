    ...
    func renderAttributeText() -> NSAttributedString {
        if joined! == false {
            ...
            let attributeText = NSMutableAttributedString {
                ...
                Space()
                AttributedText("voice_joined".localized()).foregroundColor(self.joined! == false ? Color.white : Color(0xFCF0B3)).font(.systemFont(ofSize: 13, weight: .semibold)).lineSpacing(5)
                Space()
            }
