...
public final class VoiceRoomDestoryedAlert: UIView {
    /// 30 is cancel,other is confirm
    @objc public var actionEvents: ((Int) -> Void)?
    ...
    lazy var confirm: UIButton = {
        let buttonWidth = self.frame.width - 60
        let buttonX = 30 // (self.frame.width - buttonWidth) / 2
        return UIButton(type: .custom).frame(CGRect(x: CGFloat(buttonX), y: self.title.frame.maxY + 40, width: buttonWidth, height: 40)).cornerRadius(20).textColor(.white, .normal).title(LanguageManager.localValue(key: "voice_confirm"), .normal).font(.systemFont(ofSize: 16, weight: .semibold)).setGradient([UIColor(red: 0.13, green: 0.61, blue: 1, alpha: 1), UIColor(red: 0.2, green: 0.37, blue: 1, alpha: 1)], [CGPoint(x: 0, y: 0), CGPoint(x: 0, y: 1)]).tag(31).addTargetFor(self, action: #selector(buttonAction(_:)), for: .touchUpInside)
    }()
    ...
    @objc public convenience init(frame: CGRect, title message: String, confirm text: String) {
        self.init(frame: frame)
        addSubViews([title, confirmContainer, confirm])
        title.text = message
        confirm.setTitle(LanguageManager.localValue(key: text), for: .normal)
        confirmContainer.layer.cornerRadius = 20
        confirmContainer.layer.shadowRadius = 8
