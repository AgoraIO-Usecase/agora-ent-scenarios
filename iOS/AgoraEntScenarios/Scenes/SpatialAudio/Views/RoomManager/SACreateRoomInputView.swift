...
public class SACreateRoomInputView: UIView, UITextFieldDelegate {
    var code = ""
    ...
    private let codeMessage = "spatial_voice_enter_4_digit_password".localized_spatial()

    private let nameMessage = "spatial_voice_please_set_a_name".localized_spatial()

    private var offset = CGFloat(ScreenHeight < 812 ? 150 : 120)

    var oldCenter: CGPoint = .zero

    lazy var roomName: UILabel = .init(frame: CGRect(x: 40, y: 0, width: 80, height: 20)).font(.systemFont(ofSize: 14, weight: .regular)).text("spatial_voice_room_name".localized_spatial()).textColor(.darkText).backgroundColor(.clear)
    ...
    lazy var roomNameField: UITextField = .init(frame: CGRect(x: 45, y: self.roomName.frame.maxY + 15, width: ScreenWidth - 90, height: 40)).placeholder("Sspatial_voice_set_room_name".localized_spatial()).font(.systemFont(ofSize: 18, weight: .regular)).textColor(.darkText).delegate(self)

    lazy var roomEncryption: UILabel = .init(frame: CGRect(x: self.roomName.frame.minX, y: self.roomBackground.frame.maxY + 12, width: 150, height: 20)).font(.systemFont(ofSize: 14, weight: .regular)).textColor(.darkText).text("spatial_voice_room_access".localized_spatial()).backgroundColor(.clear)

    lazy var publicChoice: UIButton = .init(type: .custom).frame(CGRect(x: self.roomEncryption.frame.minX, y: self.roomEncryption.frame.maxY + 12, width: 90, height: 32)).title("spatial_voice_public".localized_spatial(), .normal).font(.systemFont(ofSize: 14, weight: .regular)).textColor(UIColor(0x3C4267), .normal).backgroundColor(.clear).tag(21).addTargetFor(self, action: #selector(chooseEncryption(_:)), for: .touchUpInside)

    lazy var privateChoice: UIButton = .init(type: .custom).frame(CGRect(x: self.publicChoice.frame.maxX + 20, y: self.roomEncryption.frame.maxY + 12, width: 90, height: 32)).title("Spatial_voice_private".localized_spatial(), .normal).font(.systemFont(ofSize: 14, weight: .regular)).textColor(UIColor(0x3C4267), .normal).backgroundColor(.clear).tag(22).addTargetFor(self, action: #selector(chooseEncryption(_:)), for: .touchUpInside)

    lazy var pinCode: SAVerifyCodeView = .init(frame: CGRect(x: 0, y: self.publicChoice.frame.maxY + 15, width: ScreenWidth, height: self.PinHeight), codeNumbers: 4, space: 16, padding: 45)
    ...
    lazy var timeWarning: NSAttributedString = {
        ...
        space.append(NSAttributedString {
            AttributedText("spatial_voice_demo_test_desc".localized_spatial()).foregroundColor(UIColor(0x3C4267)).font(.systemFont(ofSize: 12, weight: .regular)).lineBreakeMode(.byCharWrapping).alignment(.center)
        })
        return space
    }()

    lazy var create: UIButton = {
        UIButton(type: .custom).frame(CGRect(x: 30, y: self.frame.height - CGFloat(ZTabbarHeight), width: ScreenWidth - 60, height: 48)).cornerRadius(24).title("spatial_voice_go_live".localized_spatial(), .normal).textColor(.white, .normal).font(.systemFont(ofSize: 16, weight: .semibold)).addTargetFor(self, action: #selector(createAction), for: .touchUpInside).setGradient([UIColor(red: 0.13, green: 0.608, blue: 1, alpha: 1), UIColor(red: 0.204, green: 0.366, blue: 1, alpha: 1)], [CGPoint(x: 0, y: 0), CGPoint(x: 0, y: 1)])
    }()
...
public extension SACreateRoomInputView {
    private func setupAttributes() {
        pinCode.alpha = 0
        randomName.set(image:UIImage.sceneImage(name: "random", bundleName: "VoiceChatRoomResource"), title: "spatial_voice_random".localized_spatial(), titlePosition: .right, additionalSpacing: 5, state: .normal)
        ...
        if privateChoice.isSelected {
            if let name = roomNameField.text,!name.isEmpty,!self.code.isEmpty {
                ...
                if roomNameField.text?.isEmpty ?? true {
                    superview?.superview?.makeToast("spatial_voice_no_room_name".localized_spatial(), point: superview?.superview?.center ?? .zero, title: nil, image: nil, completion: nil)
                }
            }
        } else {
