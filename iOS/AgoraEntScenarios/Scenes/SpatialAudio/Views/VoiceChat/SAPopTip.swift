...
/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the poptip.
 ...
 - `none`: No Animation
 */
public enum SAPopTipEntranceAnimation {
...
/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the poptip.
 ...
 - `none`: No Animation
 */
public enum SAPopTipExitAnimation {
...
/** Enum that specifies the type of action animation. Action animations are performed after the poptip is visible and the entrance animation completed.
 ...
 - `none`: No animation
 */
public enum SAPopTipActionAnimation {
...
private let SADefaultBounceOffset = CGFloat(8)
private let SADefaultFloatOffset = CGFloat(8)
private let SADefaultPulseOffset = CGFloat(1.1)

open class SAPopTip: UIView {
    ...
    open var isVisible: Bool { return superview != nil }
    ...
    open var tapHandler: ((SAPopTip) -> Void)?
    /// A block that will be fired when the user taps outside the poptip.
    open var tapOutsideHandler: ((SAPopTip) -> Void)?
    ...
    open private(set) var direction = SAPopTipDirection.none
    ...
    fileprivate func setup() {
        ...
        if direction.isAuto {
            var spaces: [SAPopTipDirection: CGFloat] = [:]
        ...
        if tapGestureRecognizer == nil {
            tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(SAPopTip.handleTap(_:)))
            tapGestureRecognizer?.cancelsTouchesInView = false
            addGestureRecognizer(tapGestureRecognizer!)
        }
        if shouldDismissOnTapOutside && tapToRemoveGestureRecognizer == nil {
            tapToRemoveGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(SAPopTip.handleTapOutside(_:)))
        }
        if shouldDismissOnSwipeOutside && swipeGestureRecognizer == nil {
            swipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(SAPopTip.handleSwipeOutside(_:)))
            swipeGestureRecognizer?.direction = swipeRemoveGestureDirection
        }

        if isApplicationInBackground == nil {
            NotificationCenter.default.addObserver(self, selector: #selector(SAPopTip.handleApplicationActive), name: UIApplication.didBecomeActiveNotification, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(SAPopTip.handleApplicationResignActive), name: UIApplication.willResignActiveNotification, object: nil)
        }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let path = SAPopTip.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, arrowRadius: arrowRadius, borderWidth: borderWidth, radius: cornerRadius)
    ...
    open func show(text: String, direction: SAPopTipDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        resetView()
    ...
    ///   - frame: The originating frame. The poptip's arrow will point to the center of this frame.
    ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
    open func show(attributedText: NSAttributedString, direction: SAPopTipDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        resetView()
    ...
    ///   - frame: The originating frame. The poptip's arrow will point to the center of this frame.
    ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
    open func show(customView: UIView, direction: SAPopTipDirection, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        resetView()
    ...
    // #if canImport(SwiftUI) && canImport(Combine)
        ...
        ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
        @available(iOS 13.0, *)
        open func show<V: View>(rootView: V, direction: SAPopTipDirection, in view: UIView, from frame: CGRect, parent: UIViewController, duration: TimeInterval? = nil) {
            resetView()
            ...
            let maxContentWidth = UIScreen.main.bounds.width - (edgeMargin * 2) - edgeInsets.horizontal - (padding * 2)
    ...
    fileprivate func updateBubble() {
        stopActionAnimation {
            UIView.animate(withDuration: 0.2, delay: 0, options: [.transitionCrossDissolve, .beginFromCurrentState], animations: {
                ...
                let path = SAPopTip.pathWith(rect: self.frame, frame: self.frame, direction: self.direction, arrowSize: self.arrowSize, arrowPosition: self.arrowPosition, arrowRadius: self.arrowRadius, borderWidth: self.borderWidth, radius: self.cornerRadius)
    ...
    fileprivate func show(duration: TimeInterval? = nil) {
        ...
        performEntranceAnimation {
            ...
            if let duration = duration {
                self.dismissTimer = Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(SAPopTip.hide), userInfo: nil, repeats: false)
            }
        }
    }

    @objc fileprivate func handleTap(_ gesture: UITapGestureRecognizer) {
        if customView?.frame.contains(gesture.location(in: self)) ?? false {
            return
        }
    ...
    fileprivate func performActionAnimation() {
        ...
        case let .bounce(offset):
            shouldBounce = true
            bounceAnimation(offset: offset ?? SADefaultBounceOffset)
        case let .float(offsetX, offsetY):
            floatAnimation(offsetX: offsetX ?? SADefaultFloatOffset, offsetY: offsetY ?? SADefaultFloatOffset)
        case let .pulse(offset):
            pulseAnimation(offset: offset ?? SADefaultPulseOffset)
        case .none:
            return
        }
    }
