...
@objc public enum SAChatBarStyle: Int {
    case normal = 0
    case spatialAudio = 1
}

@objc public enum SAChatBarEvents: Int {
    case mic = 0
    case handsUp = 1
    case eq = 2
    case gift = 3
...
@objc public enum SAChatBarState: Int {
    case unSelected = 1
    case selected = 2
    case disable = 3
}

public class SAChatBar: UIView, UICollectionViewDelegate, UICollectionViewDataSource {
    public var events: ((SAChatBarEvents) -> Void)?

    public var creator = false

    var handsState: SAChatBarState = .unSelected
    ...
    public lazy var toolBar: UICollectionView = .init(frame: CGRect(x: self.frame.width - (40 * CGFloat(self.datas.count)) - (CGFloat(self.datas.count) - 1) * 10 - 15, y: 0, width: 40 * CGFloat(self.datas.count) + (CGFloat(self.datas.count) - 1) * 10, height: self.frame.height), collectionViewLayout: self.flowLayout).delegate(self).dataSource(self).backgroundColor(.clear).registerCell(SAChatBarCell.self, forCellReuseIdentifier: "VoiceRoomChatBarCell").showsVerticalScrollIndicator(false).showsHorizontalScrollIndicator(false)

    override public init(frame: CGRect) {
        super.init(frame: frame)
    }

    public convenience init(frame: CGRect, style: SAChatBarStyle) {
        ...
        if eqShow {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                pop.show(customView: UILabel(frame: CGRect(x: 0, y: 0, width: 140, height: 31)).text(sceneLocalized("Try Best Agora Sound")).font(.systemFont(ofSize: 12, weight: .regular)).textAlignment(.center).backgroundColor(UIColor(0x0CA5FD)).textColor(.white), direction: .up, in: self, from: CGRect(x: self.frame.width - (style == .normal ? 110 : 55), y: self.toolBar.visibleCells[2].frame.origin.y, width: self.toolBar.visibleCells[2].frame.width, height: self.toolBar.visibleCells[2].frame.height))
            }
...
extension SAChatBar {
    ...
    @objc func refresh(event: SAChatBarEvents, state: SAChatBarState, asCreator: Bool) {
        creator = asCreator
        switch event {
        case .mic:
    ...
    private func refreshMicState(state: SAChatBarState) {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "VoiceRoomChatBarCell", for: indexPath) as? SAChatBarCell
        cell?.icon.image = UIImage.sceneImage(name: datas[indexPath.row])
        if indexPath.row == 1, creator, handsState != .selected {
            cell?.redDot.isHidden = false
        } else {
            cell?.redDot.isHidden = true
        }
        return cell ?? SAChatBarCell()
    }
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if events != nil {
            if indexPath.row == 1, handsState != .disable {
                events!(SAChatBarEvents(rawValue: indexPath.row)!)
            } else {
                events!(SAChatBarEvents(rawValue: indexPath.row)!)
            }
        }
