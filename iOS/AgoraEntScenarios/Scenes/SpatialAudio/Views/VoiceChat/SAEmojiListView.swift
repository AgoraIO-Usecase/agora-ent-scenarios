...
public class SAEmojiListView: UIView, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    lazy var emojiList: UICollectionView = .init(frame: CGRect(x: 0, y: 10, width: ScreenWidth, height: self.frame.height - 10), collectionViewLayout: self.flowLayout).registerCell(SAEmojiCell.self, forCellReuseIdentifier: "VoiceRoomEmojiCell").dataSource(self).delegate(self).backgroundColor(.white)
    ...
    override public init(frame: CGRect) {
        ...
        deleteEmoji.setImage(UIImage.sceneImage(name: "backspace_clr"), for: .normal)
        deleteEmoji.setImage(UIImage.sceneImage(name: "backspace"), for: .disabled)
...
public extension SAEmojiListView {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        SAEmojiManager.shared.emojis.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if emojiClosure != nil {
            emojiClosure!(SAEmojiManager.shared.emojis[indexPath.row])
        }
    }
}
