...
public class SAChatCell: UITableViewCell {
    var chat: SAChatEntity?

    lazy var container: UIImageView = .init(frame: CGRect(x: 15, y: 6, width: self.contentView.frame.width - 30, height: self.frame.height - 6)).image(UIImage.sceneImage(name:"chatBg")!).backgroundColor(.clear)
    ...
    func refresh(chat: SAChatEntity) {
