...
public class SAGiftsView: UIView, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    lazy var send: UIButton = {
        UIButton(type: .custom).frame(CGRect(x: self.chooseQuantity.frame.maxX, y: 0, width: 79, height: 40)).font(.systemFont(ofSize: 14, weight: .semibold)).setGradient([UIColor(0x219BFF), UIColor(0x345DFF)], [CGPoint(x: 0, y: 0), CGPoint(x: 0, y: 1)]).textColor(.white, .normal).title("spatial_voice_send".localized_spatial(), .normal).addTargetFor(self, action: #selector(sendAction), for: .touchUpInside)
    }()

    lazy var title: UILabel = .init(frame: CGRect(x: ScreenWidth / 2.0 - 30, y: 25.5, width: 60, height: 20)).textAlignment(.center).textColor(UIColor(0x040925)).font(.systemFont(ofSize: 16, weight: .semibold)).text("spatial_voice_gifts".localized_spatial())
    ...
    public convenience init(frame: CGRect, gifts: [SAGiftEntity]) {
        ...
        popview.countClosure = { [weak self] in
            ...
            self.chooseQuantity.isSelected = false
            self.gift_count = $0
            self.contribution.text = "spatial_voice_contribution_total".localized_spatial() + ": " + "\(Int(self.gift_count)! * Int(self.current?.gift_price ?? "1")!)"
        }
        current = self.gifts.first
        contribution.text = "spatial_voice_contribution_total".localized_spatial() + ": " + "1"
    }
...
public extension SAGiftsView {
    ...
    @objc internal func sendAction() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            self.contribution.text = "spatial_voice_contribution_total".localized_spatial() + ": " + "\(self.current?.gift_price ?? "1")"
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        contribution.text = "spatial_voice_contribution_total".localized_spatial() + ": " + "\(total)"
        giftList.reloadData()
    }
}
