...
public class SAGiftCell: UITableViewCell {
    ...
    lazy var giftIcon: UIImageView = .init(frame: CGRect(x: self.frame.width / 5.0 * 3, y: 0, width: self.frame.width / 5.0, height: self.contentView.frame.height)).contentMode(.scaleAspectFit).image(UIImage.sceneImage(name:"heart")!)
    ...
    func refresh(item: SAGiftEntity) {
        ...
        giftName.text = "Sent ".spatial_localized() + (gift?.gift_name ?? "")
