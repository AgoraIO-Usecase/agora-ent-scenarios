...
public class SAApplyCell: UITableViewCell {
    var agreeClosure: ((SAApply?) -> Void)?
    ...
    lazy var operation: UIButton = .init(type: .custom).frame(CGRect(x: self.contentView.frame.width - 91, y: self.avatar.center.y - 15, width: 76, height: 30)).title("spatial_voice_accept".localized(), .normal).font(.systemFont(ofSize: 14, weight: .regular)).textColor(.white, .normal).addTargetFor(self, action: #selector(apply), for: .touchUpInside)
    ...
    func refresh(item: SAApply?) {
        ...
        operation.setTitle(item?.member?.invited == true ? "spatial_voice_accepted".localized_spatial() : "spatial_voice_accept".localized_spatial(), for: .normal)
