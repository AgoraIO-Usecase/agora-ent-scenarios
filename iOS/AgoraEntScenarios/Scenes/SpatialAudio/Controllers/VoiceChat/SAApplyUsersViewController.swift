...
public class SAApplyUsersViewController: UITableViewController {
    ...
    lazy var empty: SAEmptyView = .init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 360), title: "spatial_voice_no_one_raised_hands_yet".localized_spatial(), image: nil).backgroundColor(.white)
...
extension SAApplyUsersViewController {
    ...
    private func agreeUserApply(user: SAApply?) {
        ...
        AppContext.saServiceImp().acceptMicSeatApply(chatUid: user1.uid ?? "", completion: {[weak self] error,mic  in
            ...
            self.tableView.reloadData()
            let warningMessage = (error == nil ? "spatial_voice_agree_success".localized_spatial():"spatial_voice_agree_failed".localized_spatial())
