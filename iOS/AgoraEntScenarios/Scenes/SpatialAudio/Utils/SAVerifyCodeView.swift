...
public class SAVerifyCodeView: UIView {
    var beginEdit: (() -> Void)?
    ...
    /// 输入框
    lazy var textFiled: SAVerifyCodeTextView = {
        let textFiled = SAVerifyCodeTextView(frame: CGRect(x: self.padding, y: 0, width: self.frame.width - 2 * self.padding, height: self.frame.height)).backgroundColor(.clear).textColor(.clear).delegate(self)
    ...
    /// 验证码数量
    var codeViews: [SAVerifyCodeNumberView] = []
    ...
    func initSubviews() {
        ...
        for i in 0..<inputTextNum {
            let codeNumView = SAVerifyCodeNumberView(frame: CGRect(x: padding + CGFloat(i) * (spacing + itemWidth), y: 0, width: itemWidth, height: frame.height)).isUserInteractionEnabled(false).backgroundColor(.white).cornerRadius(8).layerProperties(UIColor(0xE4E3ED), 1)
...
public extension SAVerifyCodeView {
    /// 清除所有输入
    func cleanCodes() {
        textFiled.text = ""
...
extension SAVerifyCodeView: UITextFieldDelegate {
