...
extension SAChatEntity {
    func renderAttributeText() -> NSAttributedString {
        if joined! == false {
            ...
            if (SAUserInfo.shared.currentRoomOwner?.name ?? "") == userName! {
                host.append(text)
                text = host
                string = host.string as NSString
            }
            for symbol in SAEmojiManager.shared.emojis {
                if string.range(of: symbol).location != NSNotFound {
                    ...
                    text = SAEmojiManager.shared.convertEmoji(input: text, ranges: ranges, symbol: symbol)
                }
            }
            return text
        } else {
            ...
            let attributeText = NSMutableAttributedString {
                ...
                Space()
                AttributedText("spatial_voice_joined".localized_spatial()).foregroundColor(self.joined! == false ? Color.white : Color(0xFCF0B3)).font(.systemFont(ofSize: 13, weight: .semibold)).lineSpacing(5)
                Space()
            }
