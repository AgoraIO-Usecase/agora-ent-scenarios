...
extension SoundCardSettingView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 || indexPath.row == 2 {
            ...
            cell.selectionStyle = .none
            cell.slider.value = Float(1/3.0 * gainValue)
            cell.numLable.text = String(format: "%.1ff",gainValue)
            ...
            cell.accessoryType = .disclosureIndicator
            cell.selectionStyle = .none
            let text = self.typeValue > 0 ? "\(self.typeValue)" : ""
            cell.numLable.text = "麦克风类型\(text)"
