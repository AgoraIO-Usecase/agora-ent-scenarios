...
- (void)setupView {
    ...
    logoImgView.image = [UIImage sceneImageWithName:@"" bundleName:@"KtvResource"];
    ...
    [closeBtn setImage:[UIImage sceneImageWithName:@"close_room" bundleName:@"KtvResource"] forState:UIControlStateNormal];
    [closeBtn addTarget:self action:@selector(closeBtnEvent:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:closeBtn];
    
    UIButton *moreButton = [[UIButton alloc] init];
    [moreButton setImage:[UIImage sceneImageWithName:@"icon_live_more" bundleName:@"KtvResource"] forState:(UIControlStateNormal)];
    ...
    [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_wellIcon" bundleName:@"KtvResource"] forState:UIControlStateNormal];
...
- (void)setNetworkQuality:(int)quality
{
    if(quality == 0) {
        ...
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_okIcon" bundleName:@"KtvResource"] forState:UIControlStateNormal];
        [self.networkStatusBtn setTitle:KTVLocalizedString(@"ktv_net_status_m") forState:UIControlStateNormal];
    }
    else if(quality == 2) {
        [self.networkStatusBtn setImage:[UIImage sceneImageWithName:@"ktv_network_badIcon" bundleName:@"KtvResource"] forState:UIControlStateNormal];
