...
//        dataArray.append(model)
    ...
    static func createAgoraFilterData() -> [BeautyModel] {
        ...
        // 确保所有项都有 icon，如果缺少则使用默认 icon
        // 使用确保存在的通用图标：show_beauty_ic_filter_nenbai (嫩白)、show_beauty_ic_filter_lengbai (冷白) 等
        let defaultIcons = ["show_beauty_ic_filter_nenbai", "show_beauty_ic_filter_lengbai", "show_beauty_ic_filter_yuansheng"]
        var iconIndex = 0
        dataArray.forEach { model in
            if model.icon == nil || model.icon?.isEmpty == true {
                model.icon = defaultIcons[iconIndex % defaultIcons.count]
    ...
    static func createAgoraStyleData() -> [BeautyModel] {
        ...
        // 确保所有项都有 icon，如果缺少则使用默认 icon
        // 使用确保存在的通用图标：show_beauty_ic_effect_oumei (欧美)、show_beauty_ic_effect_hunxue (混血) 等
        let defaultIcons = ["show_beauty_ic_effect_oumei", "show_beauty_ic_effect_hunxue"]
        var iconIndex = 0
        dataArray.forEach { model in
            if model.icon == nil || model.icon?.isEmpty == true {
                model.icon = defaultIcons[iconIndex % defaultIcons.count]
    ...
    static func createAgoraStickerData() -> [BeautyModel] {
        ...
        // 确保所有项都有 icon，如果缺少则使用默认 icon
        // 使用确保存在的通用图标：show_beauty_ic_sticer_zhaocaimao (招财猫贴纸)
        let defaultIcon = "show_beauty_ic_sticer_zhaocaimao"
        dataArray.forEach { model in
            if model.icon == nil || model.icon?.isEmpty == true {
