...
open class AUINetworking: NSObject {
    ...
    public func upload(model: AUIUploadNetworkModel, progress:((Float)->Void)?, completion:  ((Error?, Any?) -> Void)?) {
        ...
        let uploadReq = AF.upload(multipartFormData: { multipartFormData in
            multipartFormData.append(model.fileData, withName: model.name, fileName: model.fileName, mimeType: model.mimeType)
        ...
        uploadReq.response {[weak self] resp in
            guard let self = self else {return}
            ...
            guard let obj = obj else {
                showLogger.error("parse fail: \(String(data: data, encoding: .utf8) ?? "nil")", context: "AUINetworking")
                completion?(AUICommonError.networkParseFail.toNSError(), data)
                return
            }
            showLogger.error("request success \(String(data: data, encoding: .utf8) ?? "nil")", context: "AUINetworking")
            completion?(nil, obj)
        ...
        uploadReq.cURLDescription { url in
            showLogger.error("request: \(url)")
        }
        
        reqMap[model.uniqueId] = (uploadReq, model)
    }
