...
class EKContentView: UIView {
    ...
    private func setupOutConstraints(messageInAnchor: QLAttribute) {
        ...
        if case let .animated(animation: animation) = attributes.popBehavior {
    ...
    func animateOut(pushOut: Bool) {
        ...
        if case let .animated(animation: animation) = attributes.popBehavior, pushOut {
...
extension EKContentView {
    ...
    private func animate(by userInfo: [AnyHashable: Any]?, entrance: Bool) {
        ...
        guard case let .bind(offset: offset) = attributes.positionConstraints.keyboardRelation else {
            return
        }
