...
import io.agora.scene.joy.utils.*

class KeyboardStatusWatcher constructor(
    ...
    override fun onGlobalLayout() {
        ...
        if (heightDiff > activity.screenHeight / 3) {
            ...
            //非全屏时减去状态栏高度
            keyboardHeight = if (activity.isFullScreen) heightDiff else heightDiff - activity.statusBarHeight
            //导航栏显示时减去其高度，但横屏时导航栏在侧边，故不必扣除高度
            Log.d(TAG, "navBarHeight = ${activity.navBarHeight} \n" +
                    "hasNavBar = ${activity.hasNavBar} \n" +
                    "isNavBarShowed = ${activity.isNavBarShowed} \n" +
                    "statusBarHeight = ${activity.statusBarHeight} \n")
