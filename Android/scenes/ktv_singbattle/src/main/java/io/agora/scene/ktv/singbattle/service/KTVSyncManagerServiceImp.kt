...
class KTVSyncManagerServiceImp(
    ...
    companion object{
        @JvmField
        val kSceneId = "scene_singbattle_${BuildConfig.APP_VERSION_NAME}"
    }
    ...
    private var singBattleGameSubscribe: ((KTVServiceProtocol.KTVSubscribe, SingBattleGameModel?) -> Unit)? = null
    ...
    private var singBattleGameInfo: SingBattleGameModel? = null
    private var objIdOfSingBattleGameInfo: String? = null
    ...
    private fun resetCacheInfo(isRoomDestroyed: Boolean = false) {
        ...
        // If you have taken the mic, you need to leave the mic and clear the mic information
        if (!isRoomDestroyed) {
            seatMap.forEach {
                it.value?.let { seat ->
    ...
    override fun autoOnSeat(completion: (error: Exception?) -> Unit) {
        ...
        if (list.isEmpty()) {
            completion.invoke(java.lang.Exception(context.getString(R.string.ktv_singbattle_the_current_room_gaming_please_exit)))
        } else {
...
//        }
    ...
    override fun startSingBattleGame(
        completion: (error: Exception?) -> Unit
    ) {
        val singBattleGameModel = SingBattleGameModel(
    ...
    override fun subscribeSingBattleGame(changedBlock: (KTVServiceProtocol.KTVSubscribe, SingBattleGameModel?) -> Unit) {
        singBattleGameSubscribe = changedBlock
    }
            ...
            override fun onDeleted(item: IObject?) {
                ...
                objIdOfUserNo.forEach { entry ->
                    if (entry.value == item.id) {
                        ...
                        // TODO workaround: Do not update temporarily to prevent room reconstruction
                        //innerUpdateUserCount(userMap.size)
                        return
                    }
                }
    ...
    private fun innerAddSingBattleGameInfo(info: SingBattleGameModel,
                                           completion: (error: Exception?) -> Unit) {
        mSceneReference?.collection(kCollectionSingBattleGameInfo)
            ?.add(info, object : DataItemCallback {
                override fun onSuccess(result: IObject) {

                    singBattleGameInfo = info
                    objIdOfSingBattleGameInfo = result.id
    ...
    private fun innerUpdateSingBattleGameInfo(objectId: String,
                                              info: SingBattleGameModel,
                                              completion: (error: Exception?) -> Unit) {
        mSceneReference?.collection(kCollectionSingBattleGameInfo)
    ...
    private fun innerRemoveSingBattleGameInfo(objectId: String, completion: (error: Exception?) -> Unit) {
        mSceneReference?.collection(kCollectionSingBattleGameInfo)
            ?.delete(objectId, object : Callback {
                override fun onSuccess() {
                    runOnMainThread { completion.invoke(null) }
    ...
    private fun innerSubscribeSingBattleGameInfo(completion: () -> Unit) {
        ...
        roomSubscribeListener.add(listener)
        mSceneReference?.collection(kCollectionSingBattleGameInfo)?.subscribe(listener)
    }
