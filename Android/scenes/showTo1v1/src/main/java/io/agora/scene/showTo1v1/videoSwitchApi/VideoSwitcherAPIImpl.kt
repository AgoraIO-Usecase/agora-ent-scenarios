...
class VideoSwitcherAPIImpl constructor(private val rtcEngine: RtcEngineEx) : VideoSwitcherAPI {
    ...
    private val roomStateMap =
        Collections.synchronizedMap(mutableMapOf<RtcConnectionWrap, VideoSwitcherAPI.RoomStatus>())
    ...
    override fun preloadRoom(preloadRoomList: List<VideoSwitcherAPI.RoomInfo>) {
        Log.d(tag, "preloadRoom, preloadRoomList: $preloadRoomList")
        preloadRoomList.forEach {
            ...
            if (!hasConnection) {
                roomStateMap[RtcConnectionWrap(RtcConnection(it.channelName, it.uid))] =
                    VideoSwitcherAPI.RoomStatus.IDLE
            }
        }
    }

    override fun switchRoomState(
        newState: VideoSwitcherAPI.RoomStatus,
        roomInfo: VideoSwitcherAPI.RoomInfo,
        mediaOptions: ChannelMediaOptions?
    ) {
        Log.d(tag, "switchRoomState, newState: $newState, roomInfo: $roomInfo, roomStateMap: $roomStateMap")
            ...
            if (it.key.isSameChannel(RtcConnection(roomInfo.channelName, roomInfo.uid))) {
                ...
                when {
                    oldState == VideoSwitcherAPI.RoomStatus.IDLE && newState == VideoSwitcherAPI.RoomStatus.PREJOINED -> {
                        ...
                        // TODO eventHandler
                        val ret = rtcEngine.joinChannelEx(
                            roomInfo.token,
                            RtcConnection(roomInfo.channelName, roomInfo.uid),
                            options,
                            it.key.rtcEventHandler
                        )
                        Log.d(tag, "joinChannelEx0, roomInfo:$roomInfo, ret:$ret")
                    }
                    ...
                    oldState == VideoSwitcherAPI.RoomStatus.PREJOINED && newState == VideoSwitcherAPI.RoomStatus.JOINED -> {
                        ...
                        val ret = rtcEngine.updateChannelMediaOptionsEx(
                            options,
                            RtcConnection(roomInfo.channelName, roomInfo.uid)
                        )
                        if (ret == -8) {
                        ...
                        Log.d(tag, "updateChannelMediaOptionsEx, roomInfo:$roomInfo, ret:$ret")
                    }
                    ...
                    oldState == VideoSwitcherAPI.RoomStatus.IDLE && newState == VideoSwitcherAPI.RoomStatus.JOINED -> {
                        ...
                        mediaOptions?.let {
                            ...
                            Log.d(tag, "joinChannelEx1, roomInfo:$roomInfo, ret:$ret")
                        }
                    }
        ...
        if (newState == VideoSwitcherAPI.RoomStatus.PREJOINED) {
            ...
            mediaOptions?.let {
                ...
                val ret = rtcEngine.joinChannelEx(
                    ...
                    rtcConnection,
                    mediaOptions,
                    rtcConnectionWrap.rtcEventHandler
                )
                Log.d(tag, "joinChannelEx2, roomInfo:$roomInfo, ret:$ret")
            }
            ...
            mediaOptions?.let {
                ...
                Log.d(tag, "joinChannelEx3, roomInfo:$roomInfo, ret:$ret")
            }
        }
    ...
    inner class RtcEngineEventHandlerImpl constructor(
        ...
        override fun onError(err: Int) {
            super.onError(err)
            Log.e(
                tag, "channel ${connection.channelId} error : code=$err, message=${RtcEngine.getErrorDescription(err)}"
            )
        }
        ...
        override fun onJoinChannelSuccess(
            ...
            if (needSubscribe && needSubscribeConnection != null && channel == needSubscribeConnection?.channelId) {
                ...
                runOnUiThread {
                    ...
                    Log.d(tag, "updateChannelMediaOptionsEx2, channel:$channel, ret:$ret")
                }
            }
        ...
        override fun onLocalVideoStats(
            ...
            stats ?: return
            //Log.d("hugo", "onLocalVideoStats, dualStreamEnabled:${stats.dualStreamEnabled}, captureFrameWidth:${stats.captureFrameWidth}, captureFrameHeight:${stats.captureFrameHeight}, codecType:${stats.codecType}, hwEncoderAccelerating:${stats.hwEncoderAccelerating}")
